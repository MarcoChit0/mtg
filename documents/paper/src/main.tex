...
The two readings disagree. A deck the player assigned bracket~3 may receive a calculator bracket of 2 or 4, and the rate of disagreement is large enough to motivate this work. Neither label is the single correct answer: \(y_{\text{arch}}\) reflects the player's reading, while \(y_{\text{calc}}\) reflects one rule set. We study the misalignment rather than arbitrate it.
% ============================================================
...
Only after this descriptive comparison do we move to supervised learning. We use \(y_{\text{arch}}\) as the target and treat \(y_{\text{calc}}\) as an external reference. To test whether deck power is better captured by specific card identity or by aggregate deck structure, we build two representations from the same filtered decks. Figure~\ref{fig:representations} illustrates how the same deck is mapped into each representation: Bag of Cards encodes the decklist as a sparse count vector over the card vocabulary, while Deck Features encodes the deck as a dense vector of structural attributes.
...
Each deck is encoded as a dense vector of 105 aggregated structural attributes extracted from the Archidekt JSON. These features were handpicked to describe deck construction rather than author style: each feature belongs to a known Commander-relevant category such as mana curve, card type balance, combo density, price, popularity, or bracket-specific flags. Three of those attributes (\texttt{has\_companion}, \texttt{mainboard\_count}, and \texttt{two\_card\_combo\_singleton\_count}) are constant in the modeling subset and removed by the fold-fitted zero-variance filter, leaving 102 features actually fed to each classifier. The complete catalog, with the exact computation for each feature, is in Appendix~\ref{app:features}.
...
The divergence lift analysis separates concordant and divergent OOF rows. Cards enriched in concordant rows include \textit{Blood Moon}, \textit{Imperial Seal}, \textit{Force of Will}, and \textit{Chrome Mox}; these are cards for which the model and calculator tend to agree on high power. Rows where the model predicts below the calculator (\(\hat{y}_{\text{arch}}<y_{\text{calc}}\)) are enriched for cards such as \textit{Vorinclex, Voice of Hunger}, \textit{Expropriate}, \textit{Time Stretch}, and \textit{Time Sieve}. These cards can be strong, but their table impact depends on the surrounding deck and playgroup. What distinguishes them from the concordant set is their tempo profile: they modify the pace of the game itself, either by granting additional turns (\textit{Expropriate}, \textit{Time Stretch}, \textit{Time Sieve}) or by severely restricting opponents' resources (\textit{Vorinclex, Voice of Hunger}). The calculator tracks these effects as explicit high-impact signals, but community self-assessment tends to discount them --- players often judge their deck relative to a specific playgroup rather than as an isolated list of effects, and tempo-altering cards may feel weaker in practice than the rules text implies. This explains why a deterministic calculator and a model trained on community labels can diverge: they react to overlapping signals, but they do not encode the same notion of context.
% ============================================================
\section{Conclusion}
