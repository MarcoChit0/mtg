...
Reads pre-computed voting artefacts from experiments/voting/ (generated by
phase_e_nested_cv.py) and produces the Phase G reports. If voting artefacts
are missing or incomplete, re-computes them from individual model OOF
...
Design
------
- Hard voting (plurality) over OOF predictions of individual Phase E models.
...
- Six ensembles defined in VOTING_SPECS; voting_all dynamically includes
  all available models so it works with partial Phase E.
...
- Incrementable: re-running after new Phase E models are added updates
  voting_all automatically; other ensembles update if their members changed.
...
class VotingSpec:
    voting_id: str
    description: str
    bc_top_n: Optional[int]   # None = all available BC
    df_top_n: Optional[int]   # None = all available DF

    @property
    def uses_all(self) -> bool:
        return self.bc_top_n is None and self.df_top_n is None


VOTING_SPECS: Tuple[VotingSpec, ...] = (
    VotingSpec("voting_top3_BC",    "Top-3 modelos BC",          bc_top_n=3,    df_top_n=0),
    VotingSpec("voting_top5_BC",    "Top-5 modelos BC",          bc_top_n=5,    df_top_n=0),
    VotingSpec("voting_top3_DF",    "Top-3 modelos DF",          bc_top_n=0,    df_top_n=3),
    VotingSpec("voting_top5_DF",    "Top-5 modelos DF",          bc_top_n=0,    df_top_n=5),
    VotingSpec("voting_top3_BC_DF", "Top-3 BC + Top-3 DF",       bc_top_n=3,    df_top_n=3),
    VotingSpec("voting_all",        "Todos os modelos disponíveis", bc_top_n=None, df_top_n=None),
)
...
def rank_models(models: List[Dict], representation: str) -> List[Dict]:
    """Return models of given representation sorted by macro_f1_mean descending."""
    rep_models = [m for m in models if m["representation"] == representation]
    return sorted(rep_models, key=lambda m: m["macro_f1_mean"], reverse=True)


def resolve_members(spec: VotingSpec, models: List[Dict]) -> List[Dict]:
    """Return list of member models for a given VotingSpec."""
    bc_ranked = rank_models(models, "BC")
    df_ranked = rank_models(models, "DF")

    if spec.uses_all:
        return list(models)

    members: List[Dict] = []
    if spec.bc_top_n and spec.bc_top_n > 0:
        members.extend(bc_ranked[: spec.bc_top_n])
    if spec.df_top_n and spec.df_top_n > 0:
        members.extend(df_ranked[: spec.df_top_n])
    return members
...
def render_report(
    ...
    # --- Context ---
    lines.append(
        "Ensembles de hard-voting construídos sobre as predições out-of-fold (OOF) "
        "geradas na Fase E. Nenhum modelo é retreinado. O tie-break usa a macro-F1 "
        "média dos membros que votaram na classe; empate residual → menor rótulo.\n"
    )
...
def main(argv: Optional[Sequence[str]] = None) -> None:
    ...
    available_ids = {m["model_id"] for m in individual_models}
    ...
    for spec in VOTING_SPECS:
        ...
        # Warn about any missing expected members
        absent = []
        if spec.bc_top_n and spec.bc_top_n > 0:
            bc_ranked = rank_models(individual_models, "BC")
            if len(bc_ranked) < spec.bc_top_n:
                n_missing = spec.bc_top_n - len(bc_ranked)
                absent.append(f"{n_missing} BC model(s)")
        if spec.df_top_n and spec.df_top_n > 0:
            df_ranked = rank_models(individual_models, "DF")
            if len(df_ranked) < spec.df_top_n:
                n_missing = spec.df_top_n - len(df_ranked)
                absent.append(f"{n_missing} DF model(s)")
        if absent:
            missing_members[spec.voting_id] = absent
