...
def build_stage_plan(args: argparse.Namespace) -> List[Stage]:
    ...
    if args.skip_spot_check:
        stages.append(Stage(
            "phase_d_spot_check",
            "Implemented but skipped by request; omit --skip-spot-check to run it.",
            None,
            implemented=True,
            reason="skipped_by_--skip-spot-check",
        ))
    else:
        stages.append(Stage(
            "phase_d_spot_check",
...
def parse_args(argv: Optional[List[str]] = None) -> argparse.Namespace:
    ...
    parser.add_argument("--skip-spot-check", action="store_true", help="Skip Phase-D spot-checking.")
